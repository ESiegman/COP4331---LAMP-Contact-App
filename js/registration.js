...
async function handleRegistration(event) {
  ...
  if (login.length < 3) {
    showMessage("registerResult", "<i class='bi bi-exclamation-circle-fill me-1'></i> Username must be at least 3 characters.", "error");
    return;
  }
  ...
  if (!passwordRegex.test(password)) {
    showMessage(
      "registerResult",
      "<i class='bi bi-exclamation-circle-fill me-1'></i> Password needs 8+ characters with uppercase, lowercase, and a number.",
      "error"
    );
    return;
  }
